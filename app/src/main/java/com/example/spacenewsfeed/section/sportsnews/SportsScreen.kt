...
fun SportsScreen(
    ...
    when (sportsUiState) {
        is SportsUiState.Loading -> LoadingScreen(modifier = modifier)
        is SportsUiState.Success -> SportsScreenContent(sportsUiState.sportsNewsList, modifier = modifier)
        is SportsUiState.Error -> ErrorScreen(modifier = modifier, onRetryClicked = retryAction)
    }
...
fun SportsScreenContent(
    ...
    Surface(
        ...
        LazyColumn() {
            items(newsArticleList.articles) { sportsNews ->
                DisplayNewsCard(newsDetail = sportsNews, modifier = Modifier.padding(10.dp))
            }
        }
    }
}
