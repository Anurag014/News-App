...
fun GeneralNewsScreen(
    generalNewsUiState: GeneralNewsUiState,
    retryAction: () -> Unit,
    modifier: Modifier = Modifier
) {

    when (generalNewsUiState) {
        is GeneralNewsUiState.Loading -> LoadingScreen(modifier = modifier)
        is GeneralNewsUiState.Success -> GeneralNewsScreenContent(generalNewsUiState.generalNewsList, modifier = modifier)
        is GeneralNewsUiState.Error -> ErrorScreen(modifier = modifier, onRetryClicked = retryAction)
    }
...
fun GeneralNewsScreenContent(
    ...
    Surface(
        ...
        LazyColumn() {
            items(newsArticleList.articles) { generalNews ->
                DisplayNewsCard(newsDetail = generalNews, modifier = Modifier.padding(10.dp))
            }
        }
    }
}
